...
def conv(x, receptive_field_shape, channels_shape, stride, name):
  ...
  weights = tf.get_variable('%s/weights' % name, kernel_shape, initializer=tf.truncated_normal_initializer(stddev=.1))
  biases = tf.get_variable('%s/biases' % name, bias_shape, initializer=tf.constant_initializer(.1))
